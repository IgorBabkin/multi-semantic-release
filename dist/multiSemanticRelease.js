 ...
 */
async function multiSemanticRelease(paths, inputOptions = {}, { cwd = process.cwd(), env = process.env, stdout = process.stdout, stderr = process.stderr } = {}, flags = {}) {
    // Check params.
    ...
    const iterator = ProjectTree_1.ProjectTreeFactory.createFromArray(packages);
    while (iterator.hasNext()) {
        await Promise.all(iterator.next().map(async (pkg) => {
...
async function getPackage(path, { options: globalOptions, env, cwd, stdout, stderr }) {
    ...
    const deps = Object.keys(Object.assign(Object.assign(Object.assign(Object.assign({}, manifest.dependencies), manifest.devDependencies), manifest.peerDependencies), manifest.optionalDependencies));
...
async function releasePackage(pkg, createInlinePlugin, multiContext) {
    ...
    const options = Object.assign(Object.assign({}, pkgOptions), inlinePlugin);
