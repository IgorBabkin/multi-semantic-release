import { ProjectTreeFactory } from "./ProjectTree";
...
async function multiSemanticRelease(
	...
	const iterator = ProjectTreeFactory.createFromArray(packages);
	while (iterator.hasNext()) {
		const sourcePackages = iterator.next();
		logger.complete(`Queued sources ${sourcePackages.map(({ name }) => name).join(", ")}`);
		await Promise.all(
			sourcePackages.map(async (pkg) => {
